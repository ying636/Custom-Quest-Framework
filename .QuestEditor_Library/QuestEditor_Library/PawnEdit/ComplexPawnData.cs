...
namespace QuestEditor_Library
{
    ...
    public class PawnModWorker
    {
        ...
        {
            return value.NullOrEmpty() ? "None".Translate().ToString() : value;
        }
    ...
    public class PawnModWorker_Basic : PawnModWorker
    {
        ...
        {
            Rect row = this.DrawRowLabel(ref y, inRect, x, "ComplexPawnDefName".Translate(), 170f);
            pawnDef.defName = Widgets.TextField(new Rect(row.x, row.y, Mathf.Min(360f, row.width), 30f), pawnDef.defName);
            this.EndRow(ref y);
            row = this.DrawRowLabel(ref y, inRect, x, "ComplexPawnLabel".Translate(), 170f);
            pawnDef.label = Widgets.TextField(new Rect(row.x, row.y, Mathf.Min(360f, row.width), 30f), pawnDef.label);
            this.EndRow(ref y);
            if (this.DrawSelectRow(ref y, inRect, x, "QE_PawnKind".Translate(this.ValueOrNone(pawnDef.kindDef?.label))))
            {
                Find.WindowStack.Add(new Dialog_Select<PawnKindDef>(DefDatabase<PawnKindDef>.AllDefsListForReading, null, kind => kind.label, "SelectPawnKind".Translate(), kind => pawnDef.kindDef = kind));
            }
            if (this.DrawSelectRow(ref y, inRect, x, "PawnDataFaction".Translate() + this.ValueOrNone(pawnDef.faction?.label)))
            {
    ...
    public class PawnModWorker_NameAndBody : PawnModWorker
    {
        ...
        public override void Draw(ComplexPawnDef pawnDef, ref float y, Rect inRect, float x)
        {
            Rect row = this.DrawRowLabel(ref y, inRect, x, "Name".Translate(), 140f);
            ...
            if (this.DrawCommandText(new Rect(buttonRect.x, buttonRect.y, 180f, 30f), "RandomizePawnName".Translate()))
            {
                this.RandomizeDefName(pawnDef);
            }
            this.EndRow(ref y);
            Widgets.CheckboxLabeled(new Rect(x, y, 260f, 30f), "RandomName".Translate(), ref pawnDef.randomName);
            this.EndRow(ref y);
            if (this.DrawSelectRow(ref y, inRect, x, "PawnNameMaker".Translate(this.ValueOrNone(this.NameMakerLabel(pawnDef.nameMaker)))))
            {
                CQFEditorTools.DrawFloatMenu(DefDatabase<RulePackDef>.AllDefsListForReading, maker => pawnDef.nameMaker = maker, maker => maker.label ?? maker.defName);
            }
            row = this.DrawRowLabel(ref y, inRect, x, "BioAge".Translate(), 140f);
            Widgets.TextFieldNumeric(new Rect(row.x, row.y, 120f, 30f), ref pawnDef.bioAge, ref this.bioAgeBuffer);
            this.EndRow(ref y);
            row = this.DrawRowLabel(ref y, inRect, x, "ChronologicalAge".Translate(), 140f);
            Widgets.TextFieldNumeric(new Rect(row.x, row.y, 120f, 30f), ref pawnDef.chrAge, ref this.chrAgeBuffer);
            this.EndRow(ref y);
            if (this.DrawSelectRow(ref y, inRect, x, "Gender".Translate(pawnDef.gender.ToString().Translate())))
            {
        ...
        private void RandomizeDefName(ComplexPawnDef pawnDef)
        {
            ...
            if (maker == null)
            {
                Messages.Message("NoPawnNameMaker".Translate(), MessageTypeDefOf.CautionInput);
                return;
            }
        ...
        {
            return maker?.label ?? maker?.defName;
        }
    ...
    public class PawnModWorker_Appearance : PawnModWorker
    {
        ...
        public override void Draw(ComplexPawnDef pawnDef, ref float y, Rect inRect, float x)
        {
            if (this.DrawSelectRow(ref y, inRect, x, "CurHair".Translate(this.ValueOrNone(pawnDef.hair?.label))))
            {
                CQFEditorTools.DrawFloatMenu(DefDatabase<HairDef>.AllDefsListForReading, hair => pawnDef.hair = hair, hair => hair.label);
            }
            this.DrawColorRow(ref y, inRect, x, "SelectHairColor".Translate(), pawnDef.hairColor ?? Color.white, color => pawnDef.hairColor = this.Opaque(color));
            this.DrawColorRow(ref y, inRect, x, "SelectSkinColor".Translate(), pawnDef.skinColor ?? Color.white, color => pawnDef.skinColor = this.Opaque(color));
            if (this.DrawSelectRow(ref y, inRect, x, "CurHeadType".Translate(this.ValueOrNone(pawnDef.head?.defName))))
            {
                CQFEditorTools.DrawFloatMenu(DefDatabase<HeadTypeDef>.AllDefsListForReading, head => pawnDef.head = head, head => head.defName);
            }
            if (this.DrawSelectRow(ref y, inRect, x, "CurBodyType".Translate(this.ValueOrNone(pawnDef.bodyType?.defName))))
            {
                CQFEditorTools.DrawFloatMenu(DefDatabase<BodyTypeDef>.AllDefsListForReading, body => pawnDef.bodyType = body, body => body.defName.Translate());
            }
        }
        ...
        private void OpenColorDialog(string label, Color color, Action<Color> apply)
        {
            ...
            {
                new FloatMenuOption("Colorbase".Translate(), () => Find.WindowStack.Add(new Dialog_ChooseColor(label, color, DefDatabase<ColorDef>.AllDefsListForReading.Select(def => def.color).ToList(), apply))),
                new FloatMenuOption("Hex".Translate(), () => Find.WindowStack.Add(new Dialog_RGB(color, apply)))
            };
    ...
    public class PawnModWorker_Backstory : PawnModWorker
    {
        ...
        public override void Draw(ComplexPawnDef pawnDef, ref float y, Rect inRect, float x)
        {
            this.DrawBackstoryButton(ref y, inRect, x, "Childhood".Translate(this.ValueOrNone(pawnDef.childhood?.title)), backstory => pawnDef.childhood = backstory);
            this.DrawBackstoryButton(ref y, inRect, x, "Adulthood".Translate(this.ValueOrNone(pawnDef.adulthood?.title)), backstory => pawnDef.adulthood = backstory);
        }
        ...
        private void DrawBackstoryButton(ref float y, Rect inRect, float x, string label, Action<BackstoryDef> action)
        {
            ...
            {
                Find.WindowStack.Add(new Dialog_Select<BackstoryDef>(DefDatabase<BackstoryDef>.AllDefsListForReading, null, backstory => backstory.title, "Select".Translate(), action));
            }
        }
    }
    ...
    public class PawnModWorker_Traits : PawnModWorker
    {
        ...
        public override void Draw(ComplexPawnDef pawnDef, ref float y, Rect inRect, float x)
        {
            ...
            if (this.DrawCommandText(addRect, "Add".Translate()))
            {
                this.OpenTraitSelector(data => pawnDef.traits.Add(data));
            }
            Rect deleteRect = new Rect(addRect.xMax + 10f, y, 120f, 30f);
            if (this.DrawCommandText(deleteRect, "Delete".Translate()) && pawnDef.traits.Any())
            {
                CQFEditorTools.DrawFloatMenu(pawnDef.traits, data => pawnDef.traits.Remove(data), data => data.def?.DataAtDegree(data.degree)?.label ?? "None".Translate());
            }
            y += 42f;
            foreach (TraitData data in pawnDef.traits)
            {
                ...
                if (this.DrawTextButton(traitRect, data.def?.DataAtDegree(data.degree)?.label ?? "None".Translate()))
                {
                ...
                Widgets.Label(new Rect(traitRect.xMax + 10f, row.y + 6f, 70f, 24f), "Chance".Translate());
    ...
    public class PawnModWorker_Apparel : PawnModWorker
    {
        ...
        public override void Draw(ComplexPawnDef pawnDef, ref float y, Rect inRect, float x)
        {
            if (this.DrawCommandText(new Rect(x, y, 120f, 30f), "Add".Translate()))
            {
                ThingData data = new ThingData();
                this.OpenSelectDialog(data, DefDatabase<ThingDef>.AllDefsListForReading.Where(def => def.IsApparel).ToList(), () => pawnDef.apparels.Add(data));
            }
            if (this.DrawCommandText(new Rect(x + 130f, y, 120f, 30f), "Delete".Translate()) && pawnDef.apparels.Any())
            {
                CQFEditorTools.DrawFloatMenu(pawnDef.apparels, data => pawnDef.apparels.Remove(data), data => data.def?.label ?? "None".Translate());
            }
            y += 42f;
            foreach (ThingData data in pawnDef.apparels)
            {
                Rect row = new Rect(x, y, inRect.width - x - 20f, 36f);
                Widgets.DrawLightHighlight(row);
                Rect iconRect = new Rect(row.x + 6f, row.y + 4f, 28f, 28f);
                if (data.def?.uiIcon != null)
                {
                    Widgets.DrawTextureFitted(iconRect, data.def.uiIcon, 1f);
                }
                Rect buttonRect = new Rect(iconRect.xMax + 8f, row.y + 3f, row.width - 50f, 30f);
                if (this.DrawTextButton(buttonRect, this.ThingLabel(data)))
                {
                    this.OpenSelectDialog(data, DefDatabase<ThingDef>.AllDefsListForReading.Where(def => def.IsApparel).ToList());
                }
                y += 42f;
            }
        }
        ...
        {
            Find.WindowStack.Add(new Dialog_Select<ThingDef>(defs, def => def.uiIcon, def => def.label, "Select".Translate(), def =>
            {
        ...
        {
            Find.WindowStack.Add(new Dialog_Select<ThingDef>(GenStuff.AllowedStuffsFor(def).ToList(), stuff => stuff.uiIcon, stuff => stuff.label, "SelectStuff".Translate(), stuff =>
            {
    ...
    public class PawnModWorker_Weapon : PawnModWorker
    {
        public override void Draw(ComplexPawnDef pawnDef, ref float y, Rect inRect, float x)
        {
            ...
            if (this.DrawTextButton(new Rect(iconRect.xMax + 8f, row.y, row.width - 120f, 30f), "PawnWeapon".Translate(this.ThingLabel(pawnDef.weapon))))
            {
                this.OpenSelectDialog(pawnDef.weapon);
            }
            if (this.DrawCommandText(new Rect(row.xMax - 100f, row.y, 100f, 30f), "Delete".Translate()))
            {
                pawnDef.weapon = null;
            }
            this.EndRow(ref y);
